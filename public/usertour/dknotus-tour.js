      ...
      '<div class="tourStep popover" step="' + n + '">',
        ...
        '<div class="card">',
          '<div class="card-header">',
            !t[n].close ? '' : '<button class="tourClose close pull-right text-danger">&times;&nbsp;</button>',
          '</div>',
          '<div class="card-body">',
            '<h5 class="card-title">', t[n].title,'</h5>',
            '<p class="card-text">',t[n].content,'</p>',
          '</div>',

          '<div class="card-footer">',
            '<div>' + ucfirst(_t('step')) + ' ' + (n + 1) + ' / ' + t.length + '</div>',
            '<div class="pull-right">',
              '<div class="tourPrev btn btn-danger btn-sm">' + _t('Previous') + '</div> ',
              '<div class="tourNext btn btn-primary btn-sm" id="abc">' + _t('Next') + '</div>',
            '</div>',
          '</div>',
  ...
  return {
    ...
    next: function() {
      ...
      if (cur == t.length) {
        if (!!Tour.onfinish) {
          ...
          $.ajax({
            url:"http://127.0.0.1:8000/userGuide/finish",
            method:'get',
            data:{},
    ...
    close: function(){
      ...
      if (!!Tour.onclose) {
        ...
        $.ajax({
          url:"http://127.0.0.1:8000/userGuide/close",
          method:'get',
          data:{},
          success:function(data){
              console.log(data.success);
